export const movieList = [
  {
    "Director": {
      "Name": "J.J. Abrams",
      "Bio": "Jeffrey Jacob Abrams is an American film director, producer, and screenwriter. He is known for his work in the genres of action, drama, and science fiction.",
      "Birth": "1966"
    },
    "Genre": {
      "Name": "Science Fiction",
      "Description": "Science fiction (or sci-fi) is a film genre that uses speculative, fictional science-based depictions of phenomena that are not fully accepted by mainstream science, such as extraterrestrial lifeforms, alien worlds, extrasensory perception and time travel, along with futuristic elements."
    },
    "_id": "60b7f6508671710327fe0061",
    "Title": "Star Trek",
    "Description": "Aboard the USS Enterprise, the most-sophisticated starship ever built.",
    "ImageURL": "https://i.pinimg.com/originals/82/8b/b3/828bb3783453b22bb9f8ff59a77658da.jpg",
    "Featured": false,
    "Release": "2009",
    rating: 9,
    trailer: "https://www.youtube.com/embed/pKFUZ10Wmbw"
  },
  {
    "Director": {
      "Name": "Joss Whedon",
      "Bio": "Joseph Hill Whedon is an American film and television director, producer, screenwriter, and composer. He is the founder of Mutant Enemy Productions, co-founder of Bellwether Pictures, and is best known as the creator of several television series.",
      "Birth": "1964"
    },
    "Genre": {
      "Name": "Adventure",
      "Description": "Adventure films are a genre of film whose plots feature elements of travel. They typically involve protagonists who must leave their home or place of comfort and go to far away lands to fulfill a goal. Settings play an important role in adventure films, sometimes as big as the characters themselves."
    },
    "_id": "60b7f1ec8671710327fe005b",
    "Title": "The Avengers",
    "Description": "When Thor's evil brother, Loki (Tom Hiddleston).",
    "ImageURL": "https://i.pinimg.com/originals/0f/03/e6/0f03e6733b0cf567cc92e8e20290462f.jpg",
    "Featured": true,
    "Release": "2012",
    rating: 5,
    trailer: "https://www.youtube.com/embed/eOrNdBpGMv8"
  },
  {
    "Director": {
      "Name": "Matthew Vaughn",
      "Bio": "Matthew Allard Robert Vaughn is an English film producer, director, and screenwriter. He has produced films including Lock, Stock and Two Smoking Barrels (1998) and Snatch (2000), and directed Stardust (2007), X-Men: First Class (2011), Kingsman: The Secret Service (2014) and its sequel Kingsman: The Golden Circle (2017).",
      "Birth": "1971"
    },
    "Genre": {
      "Name": "Fantasy",
      "Description": "A fantasy story is about magic or supernatural forces, as opposed to technology as seen in science fiction."
    },
    "_id": "60b7f4788671710327fe005e",
    "Title": "Stardust",
    "Description": "To win the heart of his beloved (Sienna Miller). a young man named Tristan (Charlie Cox).",
    "ImageURL": "https://images-na.ssl-images-amazon.com/images/I/5100B4U4ylL._AC_.jpg",
    "Featured": true,
    "Release": "2007",
    rating: 5,
    trailer: "https://www.youtube.com/embed/0xhBJNWt0dw"
  },
  {
    "Director": {
      "Name": "Matthew Vaughn",
      "Bio": "Matthew Allard Robert Vaughn is an English film producer, director, and screenwriter. He has produced films including Lock, Stock and Two Smoking Barrels (1998) and Snatch (2000), and directed Stardust (2007), X-Men: First Class (2011), Kingsman: The Secret Service (2014) and its sequel Kingsman: The Golden Circle (2017).",
      "Birth": "1971"
    },
    "Genre": {
      "Name": "Adventure",
      "Description": "Adventure films are a genre of film whose plots feature elements of travel. They typically involve protagonists who must leave their home or place of comfort and go to far away lands to fulfill a goal. Settings play an important role in adventure films, sometimes as big as the characters themselves."
    },
    "_id": "60b7f5c08671710327fe0060",
    "Title": "X-Men: First Class",
    "Description": "In the early 1960s, during the height of the Cold War, a mutant named Charles Xavier (James McAvoy) meets a fellow mutant named Erik Lehnsherr (Michael Fassbender).",
    "ImageURL": "https://m.media-amazon.com/images/M/MV5BMTg5OTMxNzk4Nl5BMl5BanBnXkFtZTcwOTk1MjAwNQ@@._V1_UY1200_CR90,0,630,1200_AL_.jpg",
    "Featured": false,
    "Release": "2011",
    rating: 8,
    trailer: "https://www.youtube.com/embed/kyQKi5-k0UU"
  },
  {
    "Director": {
      "Name": "Jon Watts",
      "Bio": "John Watts is an American film director, producer, and screenwriter. His credits include directing the Marvel Cinematic Universe (MCU) superhero films Spider-Man: Homecoming and Spider-Man: Far From Home, and the upcoming Spider-Man: No Way Home; he has also signed to direct the MCU's Fantastic Four.",
      "Birth": "1981"
    },
    "Genre": {
      "Name": "Adventure",
      "Description": "Adventure films are a genre of film whose plots feature elements of travel. They typically involve protagonists who must leave their home or place of comfort and go to far away lands to fulfill a goal. Settings play an important role in adventure films, sometimes as big as the characters themselves."
    },
    "_id": "60b7f84d8671710327fe0064",
    "Title": "Spider-Man: Homecoming",
    "Description": "Thrilled by his experience with the Avengers, young Peter Parker returns home to live with his Aunt May.",
    "ImageURL": "https://media.vanityfair.com/photos/592592596736887ada186bcd/master/w_1600%2Cc_limit/spider-man-homecoming-SMH_DOM_Online_FNL_1SHT_3DRD3DIMX_AOJ_300dpi_01_rgb.jpg",
    "Featured": false,
    "Release": "2017",
    rating: 7,
    trailer: "https://www.youtube.com/embed/rk-dF1lIbIg"
    
  },
  {
    "Director": {
      "Name": "Peter Jackson",
      "Bio": "Sir Peter Robert Jackson is a New Zealand film director, producer, and screenwriter. He is best known as the director, writer, and producer of the Lord of the Rings trilogy (2001–03) and the Hobbit trilogy (2012–14), both of which are adapted from the novels of the same name by J. R. R. Tolkien. He is the third-highest-grossing film director of all time, his films having made over $6.5 billion worldwide.",
      "Birth": "1961"
    },
    "Genre": {
      "Name": "Fantasy",
      "Description": "A fantasy story is about magic or supernatural forces, as opposed to technology as seen in science fiction."
    },
    "_id": "60b7f3048671710327fe005c",
    "Title": "The Lord of the Rings: Return of the King",
    "Description": "The culmination of nearly 10 years of work and conclusion to Peter Jackson's epic trilogy.",
    "ImageURL": "http://www.limitedruns.com/media/cache/7e/6b/7e6b6743a7e45d096838abd67b2464e2.jpg",
    "Featured": false,
    "Release": "2003",
    rating: 4,
    trailer: "https://www.youtube.com/embed/r5X-hFf6Bwo"
  },
  {
    "Director": {
      "Name": "James Gunn",
      "Bio": "James Francis Gunn Jr. is an American film director, actor, producer, and screenwriter. He began his career as a screenwriter in the mid-1990s. He then began working as a director, starting with the horror-comedy film Slither (2006), and eventually moving to the superhero genre with Guardians of the Galaxy (2014), its sequel Guardians of the Galaxy Vol. 2 (2017) and The Suicide Squad (2021).",
      "Birth": "1966"
    },
    "Genre": {
      "Name": "Science Fiction",
      "Description": "Science fiction (or sci-fi) is a film genre that uses speculative, fictional science-based depictions of phenomena that are not fully accepted by mainstream science, such as extraterrestrial lifeforms, alien worlds, extrasensory perception and time travel, along with futuristic elements."
    },
    "_id": "60b7f6b58671710327fe0062",
    "Title": "Guardians of the Galaxy",
    "Description": "Brash space adventurer Peter Quill (Chris Pratt) finds himself the quarry of relentless bounty hunters after he steals an orb coveted by Ronan.",
    "ImageURL": "https://images-na.ssl-images-amazon.com/images/I/71kvH7JJFlL._AC_SY679_.jpg",
    "Featured": false,
    "Release": "2014",
    rating: 7,
    trailer: "https://www.youtube.com/embed/d96cjJhvlMA"
  },
  {
    "Director": {
      "Name": "Jon Favreau",
      "Bio": "Jonathan Kolia Favreau is an American actor and filmmaker. As a filmmaker, Favreau has been a presence in the Marvel Cinematic Universe (MCU), mainly with his collaboration with Robert Downey Jr. He directed, produced, and appeared as Happy Hogan in the films Iron Man (2008) and Iron Man 2 (2010).",
      "Birth": "1966"
    },
    "Genre": {
      "Name": "Adventure",
      "Description": "Adventure films are a genre of film whose plots feature elements of travel. They typically involve protagonists who must leave their home or place of comfort and go to far away lands to fulfill a goal. Settings play an important role in adventure films, sometimes as big as the characters themselves."
    },
    "_id": "60b7f51f8671710327fe005f",
    "Title": "Iron Man",
    "Description": "A billionaire industrialist and genius inventor, Tony Stark (Robert Downey Jr.).",
    "ImageURL": "https://www.themoviedb.org/t/p/original/aGsjgTNMSF0kswDx57YCVPqCivW.jpg",
    "Featured": false,
    "Release": "2008",
    rating: 5,
    trailer: "https://www.youtube.com/embed/8ugaeA-nMTc"
  },
  {
    "Director": {
      "Name": "Steven Spielberg",
      "Bio": "Steven Allan Spielberg is an American film director, producer, and screenwriter. He began his career in the New Hollywood era, and is one of the most commercially successful directors in history. Spielberg is the recipient of various accolades, including two Academy Awards for Best Director, a Kennedy Center honor, and a Cecil B. DeMille Award.",
      "Birth": "1946"
    },
    "Genre": {
      "Name": "Adventure",
      "Description": "Adventure films are a genre of film whose plots feature elements of travel."
    },
    "_id": "60b7f39b8671710327fe005d",
    "Title": "Jurassic Park",
    "Description": "Paleontologists Alan Grant (Sam Neill) and Ellie Sattler (Laura Dern) and mathematician.",
    "ImageURL": "https://image.posterlounge.com/images/l/1894261.jpg",
    "Featured": false,
    "Release": "1993",
    rating: 8,
    trailer: "https://www.youtube.com/embed/lc0UehYemQA"
  }
]